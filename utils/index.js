...
const checkUser = async (headers) => {
  ...
  const _getAllowedUsers = () => {
    ...
    return splittedUsers.map((user) => user.split(':')).map((splits) => ({ username: splits[0], pw: splits[1] }));
  };
  ...
  const foundAllowedUser = allowedUsers.find((allowedUser) => allowedUser.username === requestedUser.username);
  if (!foundAllowedUser) {
    return false;
  }
